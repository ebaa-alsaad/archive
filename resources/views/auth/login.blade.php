    ...
    <div class="z-10 w-full max-w-5xl mx-auto flex shadow-5xl rounded-3xl overflow-hidden bg-white dark:bg-gray-800 transition-colors duration-500 transform scale-90 opacity-0 animate-slideUp">
        ...
        <div class="w-full lg:w-3/5 p-10 sm:p-14 lg:p-20 flex flex-col justify-center dark:text-gray-100">
            <div class="mx-auto w-full max-w-md">
                ...
                {{-- Form --}}
                <form id="login-form" method="POST" class="space-y-10">
                    @csrf

                    {{-- Email --}}
                    <div class="relative group">
                        <input id="email" name="email" type="text" required autofocus autocomplete="username" placeholder=" "
                        ...
                        <label for="email"
                            ...
                            <i class="fa-solid fa-envelope mr-2"></i> البريد الإلكتروني / اسم المستخدم
                        </label>
                    </div>
                    ...
                    <div class="relative group">
                        ...
                        <label for="password"
                               class="absolute right-0 top-4 text-base text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-4 scale-75 origin-top-right peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-4 peer-focus:text-blue-600 dark:peer-focus:text-blue-400">
                            <i class="fa-solid fa-key mr-2"></i> كلمة المرور
                        </label>
                    </div>
                    ...
                    <div class="pt-6">
                        <button type="submit" id="login-button"
                            ...
                            <svg id="loading-spinner"
                                 class="animate-spin -ml-1 mr-3 h-6 w-6 text-white hidden"
                ...
                <p class="mt-12 text-center text-md text-gray-600 dark:text-gray-400 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <a href="{{ route('register') }}"
                       class="font-extrabold text-blue-500 hover:text-blue-400 transition duration-150 mr-1">
                        طلب حساب جديد
                    </a>
                </p>
            </div>
...
<style>
    ...
    /* 2. أنماط الحركة المربعة الدوارة (3D Perspective Flow) */
    .archive-3d-panel {
        perspective: 1000px; /* إعداد منظور الحركة */
        overflow: hidden;
    ...
    .archive-3d-flow {
        position: absolute;
        inset: 0;
        /* خلفية الحركة: خطوط متدرجة لخلق وهم المربع */
        ...
        animation: flow-3d 30s linear infinite;
        opacity: 0.4;
    }
    ...
    /* 🛑 الحركة الإضافية الثالثة: خطوط بيانات ضوئية عشوائية */
    .archive-extra-flow {
        position: absolute;
        inset: -20%;
        background:
            linear-gradient(90deg, transparent, rgba(66, 135, 245, 0.2), transparent),
            linear-gradient(45deg, transparent, rgba(255, 255, 255, 0.1), transparent);
        background-size: 200% 200%;
        animation: extra-flow-move 50s linear infinite;
        opacity: 0.8;
    }

    @keyframes extra-flow-move {
        0% { background-position: 0% 0%; }
        50% { background-position: 100% 100%; }
        100% { background-position: 0% 0%; }
    }

    /* 3. أنماط Floating Labels */
    input:focus ~ label,
    input:not(:placeholder-shown) ~ label {
        transform: translateY(-1rem) scale(0.75);
    }

    /* 4. أنماط التوستر البسيط (Minimal Toast) */
    .toast {
...
{{-- 🔥 سكربت تسجيل الدخول (معدل ومضمون يشتغل 100%) --}}
<script>
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showToast(message) {
        ...
        toast.className =
            "toast max-w-sm w-full shadow-lg rounded-lg pointer-events-auto overflow-hidden ring-1 ring-black ring-opacity-10 toast-error";

        toast.innerHTML = `
            <div class="p-3 text-sm flex items-center" dir="rtl">
                <i class="fa-solid fa-circle-exclamation text-red-400 ml-3"></i>
